...
/// </summary>
public sealed class NarakeetService(
    HttpClient httpClient,
    IProgressTracker progressTracker,
    ILogger<NarakeetService> logger) : ITtsService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        progressTracker.InitializeFile(fileId, textChunks.Count);
    ...
    {
        var voices = await httpClient.GetFromJsonAsync<List<NarakeetVoiceResult>>("voices");
        ...
        CancellationToken cancellationToken)
    {
        ...
        var response = await httpClient.GetAsync(taskResult.Result, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        while (true)
        {
            ...
            var response = await httpClient.GetAsync(buildTask.StatusUrl, cancellationToken);
            await EnsureSuccessStatusCode(response);
            ...
            var progress = progressTracker.UpdateProgress(
                fileId,
                progressCallback,
                chunkIndex,
                buildTaskStatus.Percent);

            logger.LogInformation("Processed chunk {ChunkIndex}/{TotalChunks} for file {FileId}. Progress: {Progress}%",
    ...
    private async Task EnsureSuccessStatusCode(HttpResponseMessage response)
    {
        ...
        logger.LogError("An error on from Narraket API. \n{Error}", error);
